...
# File serving route
@app.route('/api/uploads/<ticket_id>/<path:filename>')
@login_required
def serve_file(ticket_id, filename):
    ...
    if not ticket.attachments or filename not in ticket.attachments:
        abort(404)
        
    try:
        return send_from_directory(
            os.path.join(app.config['UPLOAD_FOLDER'], ticket_id),
            filename,
            as_attachment=True
        )
...
@app.route('/uploads/<ticket_id>/<filename>')
def uploaded_file(ticket_id, filename):
    return send_from_directory(os.path.join(app.config['UPLOAD_FOLDER'], ticket_id), filename)
